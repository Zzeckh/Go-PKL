...
async function main() {
  ...
  const classRPL = await prisma.class.create({
    data: { name: 'XII RPL 1', major: 'Rekayasa Perangkat Lunak' },
  });
  await prisma.class.create({
    data: { name: 'XII TKJ 1', major: 'Teknik Komputer Jaringan' },
  });
  ...
  console.log('   superadmin@gopkl.id → super_admin');
  console.log('   siswa@gopkl.id      → student · Kelas: XII RPL 1');
