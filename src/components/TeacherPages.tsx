...
export const TeacherMonitoring: React.FC = () => {
  ...
  const statusColor = (status: string) => {
    ...
    if (status === 'revision') return 'bg-rose-100 text-rose-700';
    return 'bg-[#FBF3E2] text-[#9A6B15]';
  };
                    ...
                    <tr key={siswa.id} className="border-b border-mist/40 transition-colors hover:bg-[#F1F4F8]/40">
                      ...
                      <td className="p-4">
                        {latestLog ? (
                          ...
                          <span className="flex items-center gap-1.5 text-[10px] font-bold bg-[#FBF3E2] text-[#9A6B15] px-2.5 py-1 rounded-full w-max">
              ...
              <div key={req.id} className="bg-white rounded-[24px] p-5 border border-mist/60 shadow-sm flex flex-col gap-4">
                <div className="flex items-start justify-between gap-4 border-b border-mist/60 pb-4">
                  ...
                  <span className={`text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full ${
                    req.type === 'Sakit' ? 'bg-rose-100 text-rose-700' : 'bg-[#FBF3E2] text-[#9A6B15]'
                  }`}>
                    {req.type}
                  </span>
                </div>
                  ...
                  {req.status === 'pending' ? (
                    <>
                      <button
                        ...
                        className="flex-1 py-3 bg-rose-50 border border-rose-200 text-rose-700 rounded-2xl text-xs font-bold flex items-center justify-center gap-2 hover:bg-rose-100 transition-colors"
                      >
                    ...
                    <div className={`w-full py-2 rounded-xl text-center border ${
                      req.status === 'approved' ? 'bg-steel/10 border-steel/20' : 'bg-rose-50 border-rose-200'
                    }`}>
                      <span className={`text-xs font-bold capitalize ${req.status === 'approved' ? 'text-steel' : 'text-rose-700'}`}>
