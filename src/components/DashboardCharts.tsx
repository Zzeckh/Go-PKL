...
const BarList: React.FC<{
  ...
  return (
    <div className="bg-white border border-mist/60 rounded-[24px] p-4 shadow-sm min-w-0">
      ...
      {items.length === 0 ? (
        <p className="text-xs font-semibold text-navy/50 py-6 text-center">
        ...
        >
          ...
          {items.map((item, index) => (
            ...
            >
              ...
              <div className="relative w-full h-36 flex items-end">
                ...
                <div
                  ...
                  style={{
                    ...
                    boxShadow:
                      '8px 8px 0 rgba(21,42,66,0.16), inset -6px 0 0 rgba(0,0,0,0.12), inset 5px 0 0 rgba(255,255,255,0.16)',
                  }}
                />
              ...
              <span
                className="w-full text-center text-[10px] font-bold text-navy/60 truncate"
                title={item.name}
              >
                {item.name}
              </span>
...
const Donut3D: React.FC<{
  ...
  const segments = values.map((item) => {
    const end =
      start +
      (item.value / Math.max(total, 1)) * 360;
      ...
      {total === 0 ? (
        ...
        <div className="flex items-center gap-4">
          ...
          >
            ...
            <div
              className="absolute inset-2 rounded-full transition-transform duration-300 hover:rotate-x-12"
              title={`Total: ${total}`}
              style={{
                background:
                  `conic-gradient(${segments.join(', ')})`,
                transform:
                  'rotateX(58deg) rotateZ(-12deg)',
                boxShadow:
                  '0 14px 0 rgba(21,42,66,0.18), 0 18px 22px rgba(21,42,66,0.12)',
              }}
            />
          ...
          <div className="space-y-2 min-w-0 flex-1">
            ...
            {values.map((item) => (
              ...
              <div
                key={item.label}
                className="flex items-center gap-2 text-[11px] font-bold text-navy/70"
              >
                ...
                <span className="ml-auto text-navy tabular-nums">
                  {item.value}
                </span>
...
export const DashboardCharts: React.FC<{
  ...
  if (error) {
    ...
    return (
      <div className="bg-navy/5 border border-navy/15 rounded-[24px] p-4 text-xs font-semibold text-navy">
  ...
  if (!stats) {
    ...
    return (
      <div className="flex items-center justify-center gap-2 p-8 text-navy/60">
  ...
  if (role === 'teacher') {
    ...
    return (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        ...
        <Donut3D
          ...
          values={[
            ...
            },
            {
              label: 'Izin',
              value:
                stats.studentStatus?.permission || 0,
              color: '#e0a458',
            },
            {
              label: 'Sakit',
              value:
                stats.studentStatus?.sick || 0,
              color: '#c96b6b',
            },
            {
              label: 'Belum Ditempatkan',
              value:
                stats.studentStatus?.notPlaced || 0,
              color: '#7c8794',
            },
          ]}
        />
        ...
        <BarList
          title="Siswa per Perusahaan Bimbingan"
  ...
  if (role === 'mentor') {
    ...
    return (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        ...
        <Donut3D
          title="Status Logbook Siswa"
          values={[
            {
              label: 'Pending',
              value:
                stats.logbookStatus?.pending || 0,
              color: '#e0a458',
            },
            {
              label: 'Approved',
              value:
                stats.logbookStatus?.approved || 0,
              color: '#2f7f95',
            },
            {
              label: 'Revision',
              value:
                stats.logbookStatus?.revision || 0,
              color: '#c96b6b',
            },
          ]}
        />
