...
export const HubinData: React.FC = () => {
  ...
  const getSiswaLogs = (siswaName: string) =>
    logEntries.filter(l => l.title.toLowerCase().includes(siswaName.split(' ')[0].toLowerCase())).slice(0, 3);
    ...
    <div className="h-full w-full flex flex-col gap-3 md:gap-4 overflow-hidden">
      ...
      <div className="flex items-center justify-between shrink-0 bg-white rounded-[24px] p-4 md:p-5 border border-mist/60 shadow-sm">
        <div className="flex items-center gap-3 md:gap-4 min-w-0">
          ...
          <div className="min-w-0">
            <h2 className="font-bold text-lg md:text-xl text-navy leading-tight truncate">Kelola Data</h2>
      ...
      <div className="flex-1 bg-white rounded-[24px] border border-mist/60 shadow-sm flex flex-col overflow-hidden min-h-0">
        <div className="px-4 md:px-5 pt-4 pb-3 shrink-0 space-y-3 border-b border-mist/60">
          <div className="bg-mist/40 p-1 rounded-[24px] flex items-center gap-1 overflow-x-auto">
            ...
            {tabConfig.filter(t => t.group === 'dalam').map(t => {
              ...
              return (
                ...
                >
                  ...
                  {t.label}
                  <span className={`text-[10px] tabular-nums ${active ? 'text-white/80' : 'text-navy/40'}`}>
                    {t.count}
                  </span>
                </button>
              );
            ...
            <span className="text-[10px] font-bold text-navy/40 uppercase tracking-widest px-2 shrink-0">Luar</span>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className={`flex justify-between items-center rounded-lg px-2.5 py-1.5 border ${
                          ...
                          <div className="flex items-center gap-1.5">
                            <MapPinned className={`w-3.5 h-3.5 ${hasCoords ? 'text-steel' : 'text-navy/50'}`} />
                            <span className={`text-[11px] font-bold ${hasCoords ? 'text-steel' : 'text-navy/60'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
              {filteredMentor.length === 0 ? (
                ...
                filteredMentor.map(m => (
                  ...
                  >
                    <div className="flex items-center gap-3 mb-3">
                      {/* ✅ FIX: avatar mentor NAVY (bukan steel) — seragam dengan siswa & guru */}
                      <div className="w-11 h-11 rounded-[10px] bg-navy text-white flex items-center justify-center font-bold text-sm shrink-0 shadow-md shadow-navy/20">
                        {getInitials(m.name)}
                      </div>
                      <div className="flex-1 min-w-0">
      ...
      {detailGuru && (
        <DetailModal
          ...
          icon={Users}
          badge={{ label: 'GURU', icon: GraduationCap, bg: 'bg-white', text: 'text-navy' }}
        >
          <div className="grid grid-cols-2 gap-2 mb-5">
      ...
      {/* ✅ FIX: modal mentor — avatar & badge navy */}
      {detailMentor && (
        <DetailModal
          ...
          icon={Briefcase}
          badge={{ label: 'MENTOR', icon: Briefcase, bg: 'bg-white', text: 'text-navy' }}
        >
          <div className="grid grid-cols-2 gap-2 mb-5">
...
const SiswaPreviewList: React.FC<{ list: any[] }> = ({ list }) => {
  ...
  return (
    <div className="space-y-2">
      {list.map(s => (
        <div key={s.id} className="p-2.5 rounded-2xl border border-mist/60 bg-white flex items-center gap-3">
...
const LogPreviewList: React.FC<{ logs: any[] }> = ({ logs }) => {
  ...
  const StatusIcon = (s: string) => {
    if (s === 'approved') return <CheckCircle2 className="w-3 h-3" />;
    if (s === 'revision') return <X className="w-3 h-3" />;
    return <Clock className="w-3 h-3" />;
  };
  return (
    <div className="space-y-2">
      {logs.map(l => (
        <div key={l.id} className="p-3 rounded-2xl border border-mist/60 bg-white">
          <div className="flex items-center justify-between gap-2 mb-1">
            <span className="text-[10px] font-bold text-navy/40 uppercase tracking-wide">{l.date}</span>
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1 ${statusPill(l.status)}`}>
...
const AddDataModal: React.FC<AddDataModalProps> = ({ onClose, activeTab, addSiswa, addPerusahaan }) => {
  ...
  const [formSiswa, setFormSiswa] = useState({ name: '', kelas: '', guruPembimbing: '', perusahaan: '' });
  const [formGuru, setFormGuru] = useState({ name: '', subject: '' });
  const [formPerusahaan, setFormPerusahaan] = useState({ name: '', address: '', quota: 5, mentor: '' });
  const [formMentor, setFormMentor] = useState({ name: '', role: '', perusahaan: '' });
    ...
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-3 sm:p-6 bg-navy/50 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-white rounded-t-[24px] sm:rounded-[24px] max-w-2xl w-full max-h-[90vh] shadow-2xl flex flex-col overflow-hidden border border-mist/60">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto custom-scrollbar p-4 sm:p-5 space-y-3">
          ...
          {formTab === 'siswa' && (
            <>
              <FormInput label="Nama Lengkap" value={formSiswa.name} onChange={v => setFormSiswa({ ...formSiswa, name: v })} required placeholder="Nama siswa" />
              <FormInput label="Kelas" value={formSiswa.kelas} onChange={v => setFormSiswa({ ...formSiswa, kelas: v })} placeholder="Contoh: XII RPL 1" />
              <FormSelect label="Guru Pembimbing" value={formSiswa.guruPembimbing} onChange={v => setFormSiswa({ ...formSiswa, guruPembimbing: v })} options={guruList.map(g => g.name)} />
              <FormSelect label="Tempat PKL (Perusahaan)" value={formSiswa.perusahaan} onChange={v => setFormSiswa({ ...formSiswa, perusahaan: v })} options={perusahaanList.map(c => c.name)} />
            </>
          )}

          {formTab === 'guru' && (
            <>
              <FormInput label="Nama Lengkap" value={formGuru.name} onChange={v => setFormGuru({ ...formGuru, name: v })} required placeholder="Nama guru" />
              <FormInput label="Mata Pelajaran / Bidang" value={formGuru.subject} onChange={v => setFormGuru({ ...formGuru, subject: v })} placeholder="Contoh: Produktif RPL" />
            </>
          )}

          {formTab === 'perusahaan' && (
            <>
              <FormInput label="Nama Perusahaan" value={formPerusahaan.name} onChange={v => setFormPerusahaan({ ...formPerusahaan, name: v })} required placeholder="Contoh: UPTD Tikomdik" />
              <FormInput label="Alamat" value={formPerusahaan.address} onChange={v => setFormPerusahaan({ ...formPerusahaan, address: v })} required placeholder="Alamat lengkap" />
              <FormInput label="Kuota Siswa" value={formPerusahaan.quota} onChange={v => setFormPerusahaan({ ...formPerusahaan, quota: Number(v) || 0 })} type="number" />
              <FormSelect label="Mentor DUDI" value={formPerusahaan.mentor} onChange={v => setFormPerusahaan({ ...formPerusahaan, mentor: v })} options={mentorList.map(m => m.name)} />
            </>
          )}

          {formTab === 'mentor' && (
            <>
              <FormInput label="Nama Lengkap" value={formMentor.name} onChange={v => setFormMentor({ ...formMentor, name: v })} required placeholder="Nama mentor" />
              <FormInput label="Role / Jabatan" value={formMentor.role} onChange={v => setFormMentor({ ...formMentor, role: v })} placeholder="Contoh: Kepala Unit" />
              <FormSelect label="Perusahaan" value={formMentor.perusahaan} onChange={v => setFormMentor({ ...formMentor, perusahaan: v })} options={perusahaanList.map(c => c.name)} />
            </>
          )}
