...
import {
  ...
  Bell, Sparkles, GraduationCap, Building2, LifeBuoy
} from 'lucide-react';
...
/* ══════════════════════════════════════════════════════
   SETTINGS (LAYOUT HEADBAR — ala Tumblr settings)
   Navigasi horizontal di atas, bukan sidebar
   ══════════════════════════════════════════════════════ */
export const SettingsIntern: React.FC = () => {
  const { userName, userRole, userCompanyName, schoolName } = useApp();
  ...
  return (
    <div className="h-full w-full flex flex-col gap-4 overflow-y-auto custom-scrollbar p-4 md:p-6">
      ...
      {/* ── HEADBAR NAV (bukan sidebar) + search ── */}
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 p-4 rounded-[16px] border border-mist/60 bg-mist/30 mb-4">
            ...
            <div className="flex-1 min-w-0 text-center sm:text-left">
              <div className="flex items-center justify-center sm:justify-start gap-2 flex-wrap">
                <p className="text-base font-bold text-navy truncate">{userName || 'User'}</p>
              ...
              <p className="text-[11px] font-semibold text-navy/50 mt-1 truncate">
                {schoolName || userCompanyName || 'Go-PKL Portal'}
              </p>
            </div>
          </div>
              ...
              <div className="flex items-center justify-between px-4 py-4 rounded-[16px] border border-mist/60 hover:bg-mist/30 transition-all">
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-[10px] text-navy/50">Terima notifikasi di perangkat</p>
                  </div>
                </div>
                <button
